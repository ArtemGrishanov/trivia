...
export default function initCoverScreen(options = {remix: null, screenTag: null, startBtnTag: null}) {
    ...
    const
        defScreenTag = '__plugin__coverScreen',
    ...
    Remix.registerTriggerAction('cover-screen:update_cover_screen', (event) => {
        ...
        if (enable) {
            if (!scr) {
                ...
                event.remix.addHashlistElement('router.screens', 0, {newElement: {backgroundColor: '#777', tags: `${defScreenTag} ${screenTag}`} });
    ...
    remix.addTrigger({
        when: { eventType: 'property_updated', condition: {prop: 'path', clause: 'EQUALS', value: 'app.coverScreen.enable'} },
        then: { actionType: 'cover-screen:update_cover_screen'}
    });
}