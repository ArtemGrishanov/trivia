    ...
    render() {
        ...
        return (
            <div className="rmx-control">
                {this.props.id &&
                    <p><a href={`#open?id=${this.props.id}`}>{`#${this.props.id}`}</a></p>
                }
                {this.props.label &&
                ...
                <div className={'rmx-control_elem_wr ' + (this.props.width <= NARROW_WIDTH ? 'narrow': '')} style={{width:this.props.width+'px', height:this.props.height+'px'}}>
                    <div className="rmx-control_elem">
                        <LayoutContainer editable={isEditMode}>
                            {childrenWithProps}
                        </LayoutContainer>
                    </div>
                </div>
                        ...
                        {selectors.map((sel) => {
                            return (
                                <div key={sel}>
                                    ...
                                    {!this.props.schema.getDescription(sel).enum &&
                                        <input ref={this.controlRefs[sel]} data-prop-name={sel} onBlur={this.onFocus} defaultValue={this.state.editedProps[sel]}></input>
                                    }
                                </div>
                            )
                        })}
