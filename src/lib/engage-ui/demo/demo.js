...
class Demo extends React.Component {
    ...
    getChild(id) {
        let r = BASE_SAMPLES ? BASE_SAMPLES.find( c => {
            return c.props.id == id
        }): '';
        if (r) return r;
        r = COMPONENTS ? COMPONENTS.find( c => {
            return c.props.id == id
        }): '';
        if (r) return r;
        r = QUIZ_SAMPLES ? QUIZ_SAMPLES.find( c => {
            return c.props.id == id
        }): '';
        return r
    }
    ...
    render() {
        ...
        return (
            <HashRouter>
                ...
                <Route exact path="/components" render={ () => <div>{nav}{this.cloneWithProps(COMPONENTS, {isEdit: this.state.isEdit})}</div>} />
                <Route exact path="/quiz" render={ () => <div>{nav}{QUIZ_SAMPLES}</div>} />
                <Route exact path="/" render={ () => <div>{nav}{BASE_SAMPLES}</div>} />
            </HashRouter>
        )
    }
}
