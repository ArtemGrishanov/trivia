...
export function getAdaptedChildrenProps(components, {
    ...
    const { rows, decorRows } = createInitialRows(components, origCntWidth, containerWidth, HORIZ_MARGIN, HORIZ_ROW_DEVIATION);
...
function createInitialRows(components, origCntWidth, containerWidth, horizMargin, horizDeviation) {
    ...
    const sortedTopLeft = components.filter( c => userDefinedNormalizedProps[c.id].displayType === 'flow')
        .sort( (c1,  c2) => {
