...
export function setData(data, forceFeedback = false, immediate = false) {
    ...
    if (editingCustomWidth) {
        // если пользователь редактирует нестандартную ширину
        Object.keys(data).forEach(path => {
            const propDescription = schema.getDescription(path)
            if (propDescription && propDescription.adaptedForCustomWidth) {
                // заменим сохранение свойства на сохранение адаптации этого свойства при нестандартной ширине
                const screenId = getScreenIdFromPath(path),
                    componentId = getComponentIdFromPath(path),
                    key = getPropNameFromPath(path)
                if (screenId && componentId && key) {
                    // это путь адаптационного свойства внутри конкретного экрана
                    data[
                        `router.screens.${screenId}.adaptedui.${state.session.size.width}.props.${componentId}.${key}`
                    ] = data[path]
                    delete data[path]
                }
            }
